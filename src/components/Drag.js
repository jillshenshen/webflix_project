  ...
  const D_enter = (e, index) => {
    ...
    if (e.target !== dragNode.current) {
      setList((preList) => {
        ...
        // change order
        let currentValue = newList[currentItem.grpI].items.splice(
          currentItem.itemI,
          1
        )[0];
  ...
  return (
    <Container dragging={dragging}>
      <div className="info">
        ...
        <h4>
          Each item in the watchlist can be dragged and dropped into other
          viewing status areas.
        </h4>
      </div>
            ...
            <div
              ...
              onDragEnter={
                dragging && !grp.items.length
                  ? (e) => D_enter(e, { grpI, itemI: 0 })
                  : null
              }
            >
              <div className="group-title">
                <h2>{grp.title}</h2>
                <h4>You can drop an item into this area.</h4>
              </div>
              {grp.items.map((item, itemI) => (
                <div
                  draggable="true"
                  onDragStart={(e) => {
                    D_start(e, { grpI, itemI });
                  }}
                  onDragEnter={(e) => {
